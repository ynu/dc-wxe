...
const renderClustersStatusTextLine = async () => {
  ...
  const cpuLine = `CPU资源：总${Math.round(
    result.cpuResource.totalSizeMHz / 1000,
  )} GHz，已使用${result.cpuResource.allocatedSizeMHz / 1000} GHz。`;
  const memLine = `内存资源：总${Math.round(
    result.memResource.totalSizeMB / 1000,
  )} GB， 已使用${Math.round(result.memResource.allocatedSizeMB / 1000)} GB。`;
  return `集群：共${cs.length}个；\n${cpuLine}\n${memLine}`;
};

const renderHostsStatusTextLine = async () => {
  const hosts = await model.hosts(siteUri);
  const result = hosts.list.reduce(
    (acc, cur) => {
